...
export class UserService {
  ...
  getUser( uid: string ,server_version: string | null ): Observable<User> {
    ...
    var url = `https://${versionPartURL}payment-dot-treattestenvironment.uc.r.appspot.com/rest/users/user-payment-details`;
  ...
  putEmail( uid: string, email: string, server_version: string ): Observable<any> {
    const versionPartURL = server_version ? `${server_version}-dot-` : '';
    var url = `https://${versionPartURL}payment-dot-treattestenvironment.uc.r.appspot.com/rest/users/set-email/${email}`;
